...
| 2026-09-22 | 4 | MRP releases: More in Common, published 2026-01-04, 2026-04-12, 2026-07-19 | [research-type/mrp/ index](https://www.moreincommon.org.uk/research-type/mrp/), individual `wp-content/uploads` xlsx files (see docs/mrp_sources.md for exact links) | `scripts/prep_more_in_common_xlsx.py` then `04_ingest_mrp_release.py` | One scenario per release (no TV split like Electoral Calculus); "Other" is a catch-all bucket | 631 constituencies (GB only, no NI) per release, 3,781–3,787 rows each | `mrp_releases` (release_id=6,7,8), `mrp_constituency_results` |
